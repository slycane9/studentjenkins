...
class TokenLoginForm(AuthenticationForm):
    def clean(self):
        ...
        if not UserXtraAuth.objects.filter(username=self.cleaned_data['username']).exists():
            ...
            if user_secrecy > 0:
                time_remaining, next_num = next(fake_token.FakeToken(user_xtra_auth.tokenkey.encode()))

                pass_len = len(self.cleaned_data['password'])
                token_code = str(next_num)
                token_len = len(token_code)

                if self.cleaned_data['password'].endswith(token_code):
                    self.cleaned_data['password'] = self.cleaned_data['password'][:pass_len - token_len]
                else:
